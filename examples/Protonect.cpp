...
/// [loop start]
  while(!protonect_shutdown && (framemax == (size_t)-1 || framecount < framemax))
  {
    ...
    unsigned num_points = depth->height*depth->width;
    float* data_as_floats = (float*)depth->data;
    for(unsigned i = 0; i < num_points; ++i) {
      float current = 1.0f - (data_as_floats[i] / 4500.0f); 
      update_elevation(landscape[i], current, 20.0f);
    }

    framecount++;
    if (framecount % 50 == 0) {
      // These are always 0 for depth frames
      std::cout << "Exposure (0.5-60.0): " << depth->exposure << std::endl;
      std::cout << "Gain: (1.0-1.5): " << depth->gain << std::endl;
      std::cout << "Gamma: (1.0-6.4): " << depth->gamma << std::endl;
      std::cout << "Bytes per pixel: " << depth->bytes_per_pixel << std::endl; // 4
      std::cout << "Format: " << depth->format << std::endl; // 2 = Float
      std::cout << "Width: " << depth->width << std::endl; // 2 = Float
      std::cout << "Height: " << depth->height << std::endl; // 2 = Float
      float min = FLT_MAX;
      float max = FLT_MIN;
      float avg = 0.0;
      for(unsigned i = 0; i < num_points; ++i) {
        ...
        avg -= avg / num_points;
        avg += current / num_points;
      }
      std::cout << "Min pixel value: " << min << std::endl;
      std::cout << "Max pixel value: " << max << std::endl;
      std::cout << "Avg pixel value: " << avg << std::endl;
    }
